...
describe('UnauthenticatedError', ({ it, describe }) => { // eslint-disable-line no-shadow
    ...
    it('creates instances inherited from the native Error', () => {
        ...
        const err = new UnauthenticatedError('test message', { cause });
        assert(err instanceof Error);
        assert(err instanceof UnauthenticatedError);
    ...
    describe('with defaults', ({ it }) => {
        it('has the default .name property', () => {
            const err = new UnauthenticatedError('test message');
            assertEqual('UnauthenticatedError', err.name);
            assertEqual(UnauthenticatedError.name, err.name);
        });

        it('has the default .code property', () => {
            const err = new UnauthenticatedError('test message');
            assertEqual('UNAUTHENTICATED_ERROR', err.code);
            assertEqual(UnauthenticatedError.CODE, err.code);
        });

        it('has the default .httpStatusCode property', () => {
            const err = new UnauthenticatedError('test message');
            assertEqual(401, err.httpStatusCode);
            assertEqual(UnauthenticatedError.HTTP_STATUS_CODE, err.httpStatusCode);
        });
    ...
    describe('with custom options', ({ it }) => {
        it('accepts a custom name', () => {
            const err = new UnauthenticatedError('test message', { name: 'CustomError' });
            assertEqual('CustomError', err.name);
        });

        it('accepts a custom code', () => {
            const err = new UnauthenticatedError('test message', { code: 'CUSTOM_ERROR_CODE' });
            assertEqual('CUSTOM_ERROR_CODE', err.code);
        });

        it('accepts a custom httpStatusCode', () => {
            const err = new UnauthenticatedError('test message', { httpStatusCode: 418 });
            assertEqual(418, err.httpStatusCode);
            assertEqual(true, err.httpError);
        });

        it('accepts a custom expected flag', () => {
            const err = new UnauthenticatedError('test message', { expected: false });
