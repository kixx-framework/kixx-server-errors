...
describe('UnsupportedMediaTypeError', ({ it, describe }) => { // eslint-disable-line no-shadow
    ...
    it('creates instances inherited from the native Error', () => {
        ...
        const err = new UnsupportedMediaTypeError('test message', { cause });
        assert(err instanceof Error);
        assert(err instanceof UnsupportedMediaTypeError);
    ...
    describe('with defaults', ({ it }) => {
        it('has the default .name property', () => {
            ...
            assertEqual('UnsupportedMediaTypeError', err.name);
            assertEqual(UnsupportedMediaTypeError.name, err.name);
        });

        it('has the default .code property', () => {
            const err = new UnsupportedMediaTypeError('test message');
            assertEqual('UNSUPPORTED_MEDIA_TYPE_ERROR', err.code);
            assertEqual(UnsupportedMediaTypeError.CODE, err.code);
        });

        it('has the default .httpStatusCode property', () => {
            const err = new UnsupportedMediaTypeError('test message');
            assertEqual(415, err.httpStatusCode);
            assertEqual(UnsupportedMediaTypeError.HTTP_STATUS_CODE, err.httpStatusCode);
        });
    ...
    describe('with custom options', ({ it }) => {
        it('accepts a custom name', () => {
            const err = new UnsupportedMediaTypeError('test message', { name: 'CustomError' });
            assertEqual('CustomError', err.name);
        });

        it('accepts a custom code', () => {
            const err = new UnsupportedMediaTypeError('test message', { code: 'CUSTOM_ERROR_CODE' });
            assertEqual('CUSTOM_ERROR_CODE', err.code);
        });

        it('accepts a custom httpStatusCode', () => {
            const err = new UnsupportedMediaTypeError('test message', { httpStatusCode: 418 });
            assertEqual(418, err.httpStatusCode);
            assertEqual(true, err.httpError);
        });

        it('accepts a custom expected flag', () => {
            const err = new UnsupportedMediaTypeError('test message', { expected: false });
    ...
    describe('when a sourceFunction is provided', ({ before, after, it }) => {
        ...
        it('captures the stack trace', () => {
            ...
            const err = new UnsupportedMediaTypeError('test message', null, sourceFunction);
